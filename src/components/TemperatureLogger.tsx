...
import { TemperatureReading, Action } from '../types';

interface TemperatureLoggerProps {
  restaurantId: string;
  onAddReading: (reading: TemperatureReading) => void;
  onAddAction: (action: Action) => void;
}

const TemperatureLogger: React.FC<TemperatureLoggerProps> = ({
  restaurantId,
  onAddReading,
  onAddAction,
}) => {
  const [type, setType] = useState<'cold' | 'hot' | 'frozen' | 'fridge'>('cold');
  ...
  const validateTemperature = (temp: number, type: string, unit: 'C' | 'F'): boolean => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!isValid) {
      // Create action for temperature violation
      const action: Action = {
        ...
        actionTaken: 'Temperature logged with violation - corrective action required',
        timestamp: new Date(),
        user,
      };
      onAddAction(action);
      setError(`Temperature out of range for ${type} food/equipment. Action has been logged!`);
    } else {
      setError('');
    }
  ...
  return (
    <div className="temperature-logger">
      ...
      <form onSubmit={handleSubmit}>
        <div>
          <label>Type:</label>
          <select value={type} onChange={(e) => setType(e.target.value as 'cold' | 'hot' | 'frozen' | 'fridge')}>
