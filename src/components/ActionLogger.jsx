...
const ACTION_GUIDANCE = {
  temperature:
    "Include the item, recorded temperature, safe limit, and what was done to make it safe.",
  hs_check:
    "Record what failed, the immediate control put in place, and who was informed.",
  checklist:
    "Note the missed task, why it was missed, and when it was completed or escalated.",
  other:
    "Describe the issue clearly and record the action taken to control the risk.",
};

const ActionLogger = ({ restaurantId, onAddAction }) => {
  const [issueType, setIssueType] = useState("temperature");
  ...
  return (
    <div className="action-logger">
      <div className="action-logger__header">
        ...
        <p>
          Record what went wrong, what was done immediately, and who completed
          the follow-up.
        </p>
      </div>

      <div className="action-logger__tip">
        <strong>Location:</strong> Restaurant {restaurantId} ·{" "}
        <strong>Issue type:</strong> {ISSUE_TYPE_LABELS[issueType]}
      </div>
      ...
      <form onSubmit={handleSubmit} className="action-logger__form">
        ...
        {successMessage && (
          <p className="action-logger__success">{successMessage}</p>
        )}

        <button type="submit" className="action-logger__submit">
