...
interface ChecklistLoggerProps {
  restaurantId: string;
...
const ChecklistLogger: React.FC<ChecklistLoggerProps> = ({
  restaurantId,
  onAddChecklist,
}) => {
  ...
  React.useEffect(() => {
    const defaultItems = (checklistType === 'opening' ? openingItems : closingItems).map((item, index) => ({
      id: index.toString(),
      item,
      checked: false,
      notes: ''
  ...
  const handleItemChange = (id: string, checked: boolean, notes: string) => {
    setItems(prev => prev.map(item =>
      item.id === id ? { ...item, checked, notes } : item
    ));
  };
            ...
            <div key={item.id} className="checklist-item">
              <label>
                <input
                  ...
                  onChange={(e) => handleItemChange(item.id, e.target.checked, item.notes || '')}
                />
                {item.item}
              </label>
              <textarea
                ...
                value={item.notes || ''}
                onChange={(e) => handleItemChange(item.id, item.checked, e.target.value)}
              />
            </div>
          ))}
        </div>
