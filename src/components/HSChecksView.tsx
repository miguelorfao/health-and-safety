...
import { HSCheck, User } from '../types';

interface HSChecksViewProps {
  hsChecks: HSCheck[];
  user: User;
}

const HSChecksView: React.FC<HSChecksViewProps> = ({ hsChecks, user }) => {
  const [filterRestaurant, setFilterRestaurant] = useState<string | 'all'>('all');
  const [filterStatus, setFilterStatus] = useState<'all' | 'passed' | 'failed'>('all');

  const isAdmin = user.role === 'admin';

  // Get unique restaurant IDs
  const restaurants = Array.from(new Set(hsChecks.map(check => check.restaurantId)));

  // Filter checks
  let filteredChecks = hsChecks;

  if (!isAdmin) {
    filteredChecks = filteredChecks.filter(check => check.restaurantId === user.restaurantId);
  }

  if (filterRestaurant !== 'all') {
    filteredChecks = filteredChecks.filter(check => check.restaurantId === filterRestaurant);
  }
      ...
      <div className="hs-filters">
        {isAdmin && (
          <div>
            ...
            <select value={filterRestaurant} onChange={(e) => setFilterRestaurant(e.target.value as any)}>
        ...
        <table className="hs-table">
          ...
          <tbody>
            {filteredChecks.map((check) => (
              <tr key={check.id} className={check.passed ? 'passed-row' : 'failed-row'}>
