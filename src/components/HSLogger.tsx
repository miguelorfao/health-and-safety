...
import { HSCheck, Action } from '../types';

interface HSLoggerProps {
  restaurantId: string;
  onAddCheck: (check: HSCheck) => void;
...
const HSLogger: React.FC<HSLoggerProps> = ({
  restaurantId,
  onAddCheck,
  onAddAction,
}) => {
    ...
    <div className="hs-logger">
      ...
      <form onSubmit={handleSubmit}>
        <div>
          ...
          <select value={checkType} onChange={(e) => setCheckType(e.target.value)} required>
        ...
        <div>
          <label>Passed:</label>
          <select value={passed ? 'yes' : 'no'} onChange={(e) => setPassed(e.target.value === 'yes')}>
